...
__global__ void matrixMultKernel(float* A, float* B, float* C, int size) {
	...
	for (int i = 0; i < size / T; i++) {
		...
		__syncthreads();
		for (int j = 0; j < T/4; j += 4) { //loop unrolling
			val += Ashm[threadIdx.y][j] * Bshm[j][threadIdx.x];
			val += Ashm[threadIdx.y][j+1] * Bshm[j+1][threadIdx.x];
			val += Ashm[threadIdx.y][j+2] * Bshm[j+2][threadIdx.x];
			val += Ashm[threadIdx.y][j+3] * Bshm[j+3][threadIdx.x];
		}
		__syncthreads();
	}
	C[row * size + col] = val;
